...
def one_versus_all_train(x_train, y_train, gradient_type):
    ...
    for school in cat_unique:
        ...
        mlr = MLR(np.ones(x_train.shape[1] + 1), alpha= 1e-1, n_cycle=1000, grandient_type= gradient_type, lambda_=0.0)
...
def split_data(filename):
    ...
    data = data_spliter(x_train.values, y_train.values.reshape(-1, 1), 0.6)
...
def init_exo(filename):
    res = pd.read_csv(filename, index_col=0).drop(columns=['First Name', 'Last Name', 'Birthday', 'Arithmancy'])
    res["Best Hand"].replace({"Left":1, "Right":2}, inplace=True)
    res["Hogwarts House"].replace({'Ravenclaw':1, 'Slytherin':2, 'Gryffindor':3, 'Hufflepuff':4}, inplace=True)
